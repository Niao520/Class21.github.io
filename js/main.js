...
//获取天气
fetch('http://v1.yiketianqi.com/api?unescape=1&version=v91&appid=43656176&appsecret=I42og6Lm&ext=&cityid=&city=')
    .then(response => response.json())
    .then(data => {
        $('#wea_text').html(data.wea)
        $('#city_text').html(data.city)
        $('#tem_night').html(data.wea_night)
        $('#tem_day').html(data.wea_day)
...
$('#switchmore').on('click', function () {
    ...
    if (shoemore && $(document).width() >= 990) {
        ...
        $("#change").html("welcome to");
        $("#change1").html("Class 21！");
    }
});
